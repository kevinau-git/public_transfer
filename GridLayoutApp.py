...
import datetime
from class_on_click_event import ClickEventHandler
...
class GridLayoutApp:
    def __init__(self, root):
        ...
        # Initialize event handler after widgets are created
        self.event_handler = ClickEventHandler(self)
        
        # Configure event bindings
        self.configure_events()
        
        # Create menu bar after event handler is initialized
        self.create_menu()
    
    def configure_events(self):
        """Configure all event bindings after event handler is created"""
        # Configure button commands
        self.submit_btn.config(command=self.event_handler.submit_data)
        self.clean_btn.config(command=self.event_handler.clean_data)
        self.execute_del_btn.config(command=self.event_handler.execute_delete)
    ...
    def create_menu(self):
        ...
        submenu1.add_command(label="Option 1", command=self.event_handler.menu_option_1_1)
        submenu1.add_command(label="Option 2", command=self.event_handler.menu_option_1_2)
        submenu1.add_command(label="Option 3", command=self.event_handler.menu_option_1_3)
        
        # Second submenu with 1 option
        submenu2 = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Tools", menu=submenu2)
        submenu2.add_command(label="Settings", command=self.event_handler.menu_option_2_1)
